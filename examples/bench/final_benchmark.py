"""
Final benchmark for DML vs RAG with synthetic data.
"""
...
def _ingest_corpus(adapter: DMLAdapter, count: int, rng: random.Random) -> dict[str, Any]:
    ...
    for idx in range(count):
        ...
        adapter.ingest(
            text,
            meta={"doc_path": f"doc_{idx}.txt", "tenant_id": "bench"},
        )
...
def _run_dml_mode(
    ...
    avg_latency = statistics.mean(retrieval_latencies) if retrieval_latencies else 0
    ...
    *,
    ...
    for corpus_size in corpus_sizes:
        for query_count in query_counts:
            ...
            with tempfile.TemporaryDirectory(prefix="dml-bench-final-") as tmpdir:
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("examples/bench/final_results"),
                        help="Output path")
