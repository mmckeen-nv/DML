"""
Comprehensive synthetic benchmarks for DML vs RAG performance.
Measures retrieval time, latency, token usage, and total costs including front-loaded ingestion.
"""
...
def _ingest_corpus(adapter: DMLAdapter, count: int, rng: random.Random) -> dict[str, Any]:
    ...
    return {
        ...
        "ingest_cost_tokens": count * 60,  # Approximate
    }
...
def _rag_ingest_docs(count: int, rng: random.Random, storage_dir: Path) -> dict[str, Any]:
    ...
    for idx in range(count):
        ...
        embeddings = []
        words = text.split()
        for word in words[:30]:  # Sample first 30 words for embedding
            embeddings.append([random.random() for _ in range(1536)])  # OpenAI dimension

        # Save to simulated vector store
...
def _run_dml_mode(
    ...
    for idx, prompt in enumerate(prompts):
        ...
        # Add query tokens (LLM generation)
        tokens.append(len(prompt.split()) + len(response.split()))

    avg_latency = statistics.mean(retrieval_latencies + generation_latencies)
    avg_retrieval_latency = statistics.mean(retrieval_latencies)
    avg_generation_latency = statistics.mean(generation_latencies)
    avg_tokens = statistics.mean(tokens) if tokens else 0.0
    avg_context_tokens = statistics.mean([t for t in tokens if t < 1000]) if tokens else 0.0
    avg_query_tokens = statistics.mean([t for t in tokens if t >= 1000]) if tokens else 0.0

    return {
        "run_id": run_id,
        "mode": mode,
        "avg_latency_ms": round(avg_latency, 3),
        "avg_retrieval_latency_ms": round(avg_retrieval_latency, 3),
...
def _run_rag_mode(
    storage_dir: Path,
    prompts: Iterable[str],
    mode: str,  # "semantic", "literal", or "hybrid"
    run_id: int,
) -> dict[str, Any]:
    ...
    for corpus_size in corpus_sizes:
        for query_count in query_counts:
            for run_id in range(runs):
                ...
                with tempfile.TemporaryDirectory(prefix="dml-bench-comprehensive-") as tmpdir:
...
def analyze_results(results: List[dict[str, Any]]) -> dict[str, Any]:
    ...
    for corpus_size in [100, 1000, 10000]:
        ...
        for mode in DEFAULT_MODES + [("rag", "semantic"), ("rag", "literal"), ("rag", "hybrid")]:
            ...
            if mode_results:
                avg_tokens = statistics.mean([r["avg_tokens"] for r in mode_results])
                avg_latency = statistics.mean([r["avg_latency_ms"] for r in mode_results])
                avg_context = statistics.mean([r.get("avg_context_tokens", 0) for r in mode_results])
                avg_query = statistics.mean([r.get("avg_query_tokens", 0) for r in mode_results])

                analysis[corpus_size][mode_type] = {
                    "avg_tokens": round(avg_tokens, 2),
                    "avg_latency_ms": round(avg_latency, 3),
                    "avg_context_tokens": round(avg_context, 2),
                    "avg_query_tokens": round(avg_query, 2),
...
def write_results(results: List[dict[str, Any]], analysis: dict[str, Any], path: Path) -> None:
    ...
    fieldnames = [
        ...
        "avg_tokens", "avg_context_tokens", "avg_query_tokens",
    ]
            ...
            if mode_type in analysis[corpus_size]:
                ...
                for mode_name, metrics in analysis[corpus_size][mode_type].items():
                    summary_path.write_text(
                        ...
                        f"context={metrics['avg_context_tokens']:8.2f}, "
                        f"query={metrics['avg_query_tokens']:8.2f}\n"
                    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Comprehensive DML vs RAG benchmark")
    parser.add_argument("--corpus-sizes", type=int, nargs="+", default=[100, 1000, 10000],
                        help="Corpus sizes to test")
    parser.add_argument("--query-counts", type=int, nargs="+", default=[10, 50, 100],
    ...
    parser.add_argument("--output", type=Path, default=Path("examples/bench/comprehensive_results"),
