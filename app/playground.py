...
def _initialise_adapter() -> DMLAdapter:
    storage_dir = Path(st.session_state.get("storage_dir", "./data/playground"))
    storage_dir.mkdir(parents=True, exist_ok=True)
    adapter = DMLAdapter(
        config_overrides={"storage_dir": str(storage_dir)},
...
if "adapter" not in st.session_state:
    st.session_state["adapter"] = _initialise_adapter()

adapter: DMLAdapter = st.session_state["adapter"]

st.sidebar.header("Ingestion")
with st.sidebar:
    uploaded = st.file_uploader("Upload text or markdown", type=["txt", "md", "log", "json"])
    if uploaded is not None:
        text = uploaded.read().decode("utf-8", errors="ignore")
        adapter.ingest(text, meta={"doc_path": uploaded.name})
        st.success(f"Ingested {uploaded.name}")
    if st.button("Reset lattice"):
        adapter.close()
        st.session_state["adapter"] = _initialise_adapter()
...
if st.button("Run retrieval", type="primary") and prompt.strip():
...
if result:
    with col_context:
        ...
        st.caption(json.dumps(result.get("source_docs", []), indent=2))
    with col_stats:
