...
Current continuation: [September 24 handoff](production-handoff-2026-09-24.md). M7 remains open: attempt 7 completed 8/9 successfully but failed required supersession; attempt 8 is being prepared. Earlier dated status below is historical.
...
| 7 | Wire the live-agent semantic and outcome harness | **IN PROGRESS**: attempt 7 completed all nine tasks, passed eight, and failed required model-owned supersession. Both replays rejected it; raw evidence became unavailable after workspace rollback. Attempt 8 changes generic public tool guidance and requires fresh source validation, admission, full live evidence, independent review and CI. | Run pinned tool-driven agent episodes with task verifiers, raw events and failure-inclusive cost/quality metrics; cover the adversarial semantic cases and incorrect-retrieval feedback loops. Distinguish live outcomes from deterministic state checks and offline retrieval smoke. | 4, 6 |
