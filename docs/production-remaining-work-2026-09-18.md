...
| 7 | Wire the live-agent semantic and outcome harness | **IN PROGRESS**: retrieval metadata published in3ba0570; CI332 passed19/20, with Linux concurrency filesystem admission rejected. Reviewed CI repair and fresh pinned model execution in progress; attempt7 not launched. | Run pinned tool-driven agent episodes with task verifiers, raw events and failure-inclusive cost/quality metrics; cover the adversarial semantic cases and incorrect-retrieval feedback loops. Distinguish live outcomes from deterministic state checks and offline retrieval smoke. | 4, 6 |
