...
| Active milestone 7 work | Qualify the paired-read source correction with fresh CI and admission, then run the first complete attempt 14 model campaign and both replays. CI341 original failure remains retained; no identical job retry or model campaign retry occurred. Attempt 13 retains both failed tasks. |
...
| 7 | Wire the live-agent semantic and outcome harness | **IN PROGRESS**: attempt 14 original source was blocked by CI341 before model generation. A narrow journal-read correction passes local impact and unchanged256-client tests; fresh CI/admission and the full model campaign remain required. Both attempt 13 task failures remain unresolved. | Run pinned tool-driven agent episodes with task verifiers, raw events and failure-inclusive cost/quality metrics; cover the adversarial semantic cases and incorrect-retrieval feedback loops. Distinguish live outcomes from deterministic state checks and offline retrieval smoke. | 4, 6 |
