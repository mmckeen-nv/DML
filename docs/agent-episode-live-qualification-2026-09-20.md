...
Current continuation: [handoff updated September 25](production-handoff-2026-09-24.md). **M7 remains open.** Attempt 11 completed nine terminals on `e08c38e3`: eight verifier successes, 19 model calls, **20,576 known tokens**, no unknown usage or effects. Both actual sequential replays match and reject only verified model-owned supersession. The [completion review](artifacts/agent-episode-live-completion-grader-review-attempt-11-isolated-2026-09-25.json) releases only attempt 11's completed freeze and closes its fixed guidance hypothesis as failed. [CI 338](https://github.com/mmckeen-nv/DML/actions/runs/36083908802) passed 20/20 first-attempt jobs and received independent 9.6/10 acceptance; CI does not override live rejection.

Attempt 12 implements a separately reviewed Qwen3-1.7B BF16, exact-original-shard, explicit non-thinking runtime comparison in a new checkout. The first system-message bytes, old profiles, all 975 existing cases, tools, grammar, tasks, verifier, budgets and gates remain unchanged. [Full validation](artifacts/agent-episode-attempt-12-isolated-validation-2026-09-25-r2.json) passed **1,183 cases** (975 retained plus 208 new), zero skips/failures/errors, all three strict lint scopes and the maintained 82-file mypy scope, with all 445 source hashes unchanged. The first combined run's two typing errors after 859 passing cases remain retained; a narrow typing correction preceded this fresh complete run. The [source review](artifacts/agent-episode-attempt-12-integrated-grader-review-2026-09-25.json) covers that exact corrected source. Current-source CI, disk preflight and reviewed acquisition, actual tensor proof/admission/freeze and one full campaign with both replays remain pending. No successor capability or model admission is claimed. Original attempt 8 stays frozen/incomplete; prior historical gaps remain disclosed. Raw logs stay local.

## Historical attempt 11 preparation

Current continuation: [handoff updated September 25](production-handoff-2026-09-24.md). **M7 remains open.** Attempt 10 completed nine task terminals on `443f444d`: eight verifier successes, nine measurable finals across eight intents, 19 model calls and **18,918 known tokens**, with no unknown usage or effects. The model received the trusted validation error but then finalized without the required supersession and without evidence IDs. Both completed sequential replays are byte-identical and reject only `verified_model_owned_supersession`. The [completion review](artifacts/agent-episode-live-completion-grader-review-attempt-10-isolated-2026-09-25.json) retains the failed first reviewer-launcher setup separately, rejects qualification, and releases only attempt 10's freeze.

[CI 337](https://github.com/mmckeen-nv/DML/actions/runs/36080001253) passed 20/20 jobs on its first attempt. [Independent CI review](artifacts/agent-episode-ci-337-grader-review-2026-09-25.json) accepted 9.6/10 for exact-source evidence; the mandatory model lane passed all 891 cases without skips. Source CI does not override the failed live gate. Attempt 9 remains separately rejected, and original attempt 8 stays incomplete and frozen. Historical attempts 5–7 retain their evidence gaps.

Attempt 11 implements one fixed generic recovery-guidance paragraph under explicit profile `qwen2-action-json-recovery-v3`. This is an uncertain policy experiment following observed model instruction-following failure, not a demonstrated runtime repair. Old v1/v2 policies and identities remain exact; mechanics, grammar, public tool descriptions, weights, tasks, verifier, limits and gates remain unchanged. No further wording iterations belong to this hypothesis if it fails. Independent source review accepted 9.6/10. [Full validation](artifacts/agent-episode-attempt-11-isolated-validation-2026-09-25.json) passed all **975 cases** (925 retained plus 50 new), zero skips/failures/errors, both strict lint scopes and maintained mypy, with all 436 source hashes unchanged. Fresh exact-source CI, admission/freeze, one full trained campaign and two sequential replays remain required. No attempt-11 model outcome is claimed. Raw logs stay local; public evidence remains compact.
