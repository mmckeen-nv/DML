"""Adversarial admission for original two-shard CPU/BF16 Qwen3 artifacts."""
...
@pytest.fixture
def tiny(tmp_path):
    return create_qwen3_snapshot(tmp_path / "snapshot")
...
def test_private_original_shards_preserve_bytes_and_isolate_source(tiny):
    ...
    with snapshot.verify_qwen3_snapshot(tiny.path) as verified:
        private = verified.path
        assert private != tiny.path
        assert verified.identity.model_window_tokens == 4096
        assert verified.identity.runtime_identity.startswith("dml-qwen3-model-input-runtime-v1:")
        state = snapshot.load_qwen3_state(private, verified.config, verified.index)
        assert len(state) == 25
        records = snapshot.validate_state(state, verified.config)
        assert len(records) == 25 and all(row["source_sha256"] == row["target_sha256"] for row in records)
...
def test_each_artifact_fails_closed(tiny, tmp_path, name, fault):
    ...
    with pytest.raises((SnapshotVerificationError, OSError)):
        snapshot.verify_qwen3_snapshot(tiny.path)
...
@pytest.mark.parametrize("field,value", [
    ("architectures", ["Qwen2ForCausalLM"]), ("torch_dtype", "float32"), ("attention_bias", True),
    ("rope_scaling", {}), ("sliding_window", 4096), ("use_sliding_window", True),
    ("num_hidden_layers", 29), ("head_dim", 7), ("num_key_value_heads", 3),
    ("max_window_layers", 1), ("max_position_embeddings", 40961), ("tie_word_embeddings", False),
...
@pytest.mark.parametrize("fault", ["missing", "extra", "dtype", "shape", "nan", "positive_inf", "negative_inf", "head_value", "head_signed_zero", "noncontiguous", "meta"])
def test_full_tensor_proof_rejects_malformed_or_unequal_original_payloads(tiny, fault, monkeypatch):
    import torch
    config = json.loads((tiny.path / "config.json").read_bytes())
    index = json.loads((tiny.path / "model.safetensors.index.json").read_bytes())
    state = snapshot.load_qwen3_state(tiny.path, config, index)
    ...
    if fault == "missing":
        ...
        state["model.embed_tokens.weight"] = state["model.embed_tokens.weight"].clone()
