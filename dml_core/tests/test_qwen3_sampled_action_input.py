"""Fixed-seed sampling mechanics on tiny weights, never a model-quality claim."""
...
from daystrom_dml.contracts.agent_episode import (
    QWEN3_CONSUMER_PROFILE, QWEN3_SAMPLED_CONSUMER_PROFILE, initial_messages, parse_agent_action,
)
...
from daystrom_dml.services.agent_action_grammar import (
    ActionLogitsProcessor, action_schema, compile_action_grammar, policy_identity,
)
from daystrom_dml.services.model_input import ModelInputExecutionError, _json_bytes
from daystrom_dml.services.qwen3_action_input import (
    LocalQwen3ActionInputConsumer, _sampled_cpu_rng, constrained_identity, sampling_policy_identity,
)
...
def consumer(snapshot, grammar_runtime):
    with LocalQwen3ActionInputConsumer(snapshot.path, consumer_profile=QWEN3_SAMPLED_CONSUMER_PROFILE) as instance:
        yield instance


def _messages(prompt='Read the requested memory.'):
    return initial_messages(prompt, consumer_profile=QWEN3_SAMPLED_CONSUMER_PROFILE)
...
def test_sampled_identity_is_explicit_and_old_literal_identity_is_unchanged():
    ...
    old = constrained_identity(base, consumer_profile=QWEN3_CONSUMER_PROFILE)
    assert old.runtime_identity == 'dml-qwen3-action-runtime-v1:da3f43217de5b657a70a390b33663bed1d37d1e65aae27cdd8f2385887bbe420'
    sampled = constrained_identity(base, consumer_profile=QWEN3_SAMPLED_CONSUMER_PROFILE)
    payload = {'consumer_profile': QWEN3_SAMPLED_CONSUMER_PROFILE, 'base_runtime_identity': base.runtime_identity,
               **policy_identity(), 'inherited_guidance_policy': recovery_guidance_identity(),
               'sampling_policy': sampling_policy_identity()}
    assert sampled.runtime_identity == 'dml-qwen3-action-runtime-v2:' + hashlib.sha256(_json_bytes(payload)).hexdigest()
    assert sampled != old
...
def test_exact_effective_config_has_only_declared_sampling_changes(consumer, snapshot):
    with LocalQwen3ActionInputConsumer(snapshot.path, consumer_profile=QWEN3_CONSUMER_PROFILE) as legacy:
        expected = legacy._generation_config(31).to_dict()
    expected.update(do_sample=True, temperature=0.7, top_p=0.8, top_k=20, min_p=0.0)
    ...
    assert config.typical_p == 1.0 and config.epsilon_cutoff == config.eta_cutoff == 0.0
...
def test_actual_a_b_a_sampling_ignores_nonce_ambient_rng_and_consumer_order(consumer, snapshot):
    ...
    with LocalQwen3ActionInputConsumer(snapshot.path, consumer_profile=QWEN3_SAMPLED_CONSUMER_PROFILE) as other:
...
def test_profile_crossing_refuses_authenticated_artifacts(consumer, snapshot, monkeypatch, direction):
    with LocalQwen3ActionInputConsumer(snapshot.path, consumer_profile=QWEN3_CONSUMER_PROFILE) as old:
