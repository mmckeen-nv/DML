...
from daystrom_dml.services import qwen2_bf16_pretrained_snapshot as prepare
...
def staged(tmp_path, monkeypatch):
    ...
    fixture = create_qwen2_bf16_snapshot(stage / "bundle", context_window=32768)
...
def test_preparation_retains_every_original_inode_and_uses_one_private_copy(staged, tmp_path, monkeypatch):
    ...
    report = prepare.prepare_qwen2_bf16_snapshot(stage, target)
    ...
    assert report["tied_head"] == {
        "source": "model.embed_tokens.weight", "target": "lm_head.weight", "source_head_present": False,
        "source_embedding_retained": True, "materialization": "loader-alias-same-storage",
    }
    config = json.loads(before["config.json"][1])
    assert {record["name"] for record in report["learned_tensors"]} == snapshot.expected_shapes(config).keys()
    assert "lm_head.weight" not in {record["name"] for record in report["learned_tensors"]}
    assert report["license"]["source_file"] == "LICENSE"
    assert report["license"]["source_revision"] == report["source_revision"]
    assert report["license"]["usage"] == "research-evaluation-only"
    ...
    with snapshot.verify_qwen2_bf16_snapshot(target / "bundle") as verified:
...
def test_every_official_pin_is_checked_before_generated_files(staged, tmp_path, name, fault):
    ...
    with pytest.raises((prepare.PretrainedSnapshotError, SnapshotVerificationError, OSError)):
        prepare.prepare_qwen2_bf16_snapshot(stage, tmp_path / "target")
...
def test_preexisting_destination_is_never_overwritten(staged, tmp_path, kind):
    ...
    with pytest.raises(prepare.PretrainedSnapshotError, match="already exists"):
        prepare.prepare_qwen2_bf16_snapshot(stage, target)
...
def test_failed_preparation_preserves_all_sources_and_cleans_only_generated_metadata(staged, tmp_path, monkeypatch, failure):
    ...
    monkeypatch.setattr(snapshot if failure == "tensor" else prepare,
                        "load_qwen2_bf16_state" if failure == "tensor" else "_publish_directory", fail)
...
def test_extra_stored_head_never_publishes_even_with_self_consistent_source_pins(staged, tmp_path, monkeypatch):
    from safetensors.torch import load_file, save_file
    stage, pins = staged
    index_path = stage / "bundle" / "model.safetensors.index.json"
    index = json.loads(index_path.read_bytes())
    path = stage / "bundle" / index["weight_map"]["model.embed_tokens.weight"]
    values = load_file(str(path))
    values["lm_head.weight"] = values["model.embed_tokens.weight"].clone()
    save_file(values, str(path))
    index["weight_map"]["lm_head.weight"] = path.name
    index["metadata"]["total_size"] += values["lm_head.weight"].numel() * values["lm_head.weight"].element_size()
    index_path.write_text(json.dumps(index))
    for changed in (path, index_path):
        pins[changed.name] = (changed.stat().st_size, hashlib.sha256(changed.read_bytes()).hexdigest())
    monkeypatch.setattr(prepare, "SOURCE_FILE_PINS", pins)
    with pytest.raises(SnapshotVerificationError, match="index coverage"):
        prepare.prepare_qwen2_bf16_snapshot(stage, tmp_path / "target")
...
def test_source_profile_is_captured_before_io(staged, tmp_path, monkeypatch):
    ...
    result = prepare.prepare_qwen2_bf16_snapshot(stage, tmp_path / "target")
