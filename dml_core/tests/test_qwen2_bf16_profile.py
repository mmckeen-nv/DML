"""Qwen2 BF16 routing and replay controls; no learned live generation."""
...
SAMPLED = QWEN2_BF16_SAMPLED_CONSUMER_PROFILE
...
class SampledScriptedConsumer(Qwen3ScriptedConsumer):
    ...
    def compile(self, messages, tools, *, output_reserved_tokens):
        ...
        return replace(artifact, identity=replace(artifact.identity,
            runtime_identity="dml-qwen2-bf16-action-runtime-v1:" + "d" * 64))


@pytest.mark.parametrize("profile", [SAMPLED])
def test_qwen2_bf16_profile_routes_to_explicit_consumer(monkeypatch, profile):
    from daystrom_dml.services import qwen2_bf16_action_input
    ...
    monkeypatch.setattr(qwen2_bf16_action_input, "LocalQwen2BF16ActionInputConsumer", construct)
...
def test_unknown_profile_never_dispatches_or_replays(monkeypatch, profile):
    ...
    calls = []
    monkeypatch.setattr(qwen2_bf16_action_input, "LocalQwen2BF16ActionInputConsumer", lambda *a, **kw: calls.append(kw))
...
def test_sampled_cli_preserves_every_failure_and_binds_complete_bf16_sources(tmp_path, monkeypatch):
    ...
    assert set(sources) != set(cli._source_digests(consumer_profile=QWEN3_CONSUMER_PROFILE))
    assert len(sources) == 19 and len(cli._source_digests()) == 15
    assert all("daystrom_dml.services." + name in sources for name in (
        "qwen2_bf16_pretrained_snapshot", "qwen2_bf16_model_snapshot", "qwen2_bf16_model_input", "qwen2_bf16_action_input"))
...
def sampled_campaign(tmp_path_factory, grammar_runtime):
    from daystrom_dml.services.qwen2_bf16_action_input import LocalQwen2BF16ActionInputConsumer
    from qwen2_bf16_model_input_fixture import create_qwen2_bf16_snapshot

    directory = tmp_path_factory.mktemp("qwen2-bf16-synthetic-campaign")
    snapshot = create_qwen2_bf16_snapshot(directory / "snapshot", context_window=8192)
    with LocalQwen2BF16ActionInputConsumer(snapshot.path, consumer_profile=SAMPLED) as compiler:
...
def test_sampled_scripted_campaign_replays_original_corpus_policy_and_gates(sampled_campaign):
    ...
    assert all(tokenizer.decode(request["compiled"]["input_ids"], skip_special_tokens=False,
        clean_up_tokenization_spaces=False).endswith("<|im_start|>assistant\n") for request in requests)


@pytest.mark.parametrize("boundary", ["spec", "report", "start", "terminal", "summary", "compiled"])
def test_sampled_campaign_cannot_be_relabelled_qwen3(sampled_campaign, boundary):
    (campaign, spec, identity, tokenizer), _ = sampled_campaign
...
@pytest.mark.parametrize("profile,prefix", [
    ...
    (QWEN3_CONSUMER_PROFILE, "dml-qwen2-bf16-action-runtime-v1:"),
...
def test_sampled_file_replay_binds_shards_identity_and_exact_sources(sampled_campaign, tmp_path):
    from daystrom_dml.services.qwen2_bf16_model_snapshot import REQUIRED_FILES
