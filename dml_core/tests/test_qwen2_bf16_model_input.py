"""Tiny Qwen2BF16 BF16 runtime controls; no trained model capability evidence."""
...
from daystrom_dml.services.qwen2_bf16_model_input import LocalQwen2BF16InputConsumer, _raw_equal
from daystrom_dml.services.qwen2_bf16_model_snapshot import QWEN2_BF16_CHAT_TEMPLATE, SHARD_FILES, expected_shapes
from qwen2_bf16_model_input_fixture import create_qwen2_bf16_snapshot


@pytest.fixture(scope='module')
def snapshot(tmp_path_factory):
    return create_qwen2_bf16_snapshot(tmp_path_factory.mktemp('tiny-qwen2_bf16-input'), context_window=8192)


@pytest.fixture
def consumer(snapshot):
    with LocalQwen2BF16InputConsumer(snapshot.path) as instance:
        yield instance


def test_qwen2_bf16_loads_every_bf16_source_bit_and_only_declared_f32_rotary(snapshot, consumer):
    from safetensors.torch import load_file
    state = {}
    for shard in SHARD_FILES:
        state.update(load_file(str(snapshot.path / shard)))
    loaded = consumer._model.state_dict()
    assert loaded.keys() == state.keys() | {'lm_head.weight'}
    assert state.keys() == expected_shapes(consumer._snapshot.config).keys()
...
def test_qwen2_bf16_loader_constructs_on_meta_without_random_cpu_weights(snapshot, monkeypatch):
    from transformers import Qwen2ForCausalLM
    original = Qwen2ForCausalLM.__init__
    seen = []
    def checked(self, *args, **kwargs):
        original(self, *args, **kwargs)
        seen.append({p.device.type for p in self.parameters()})
    monkeypatch.setattr(Qwen2ForCausalLM, '__init__', checked)
    with LocalQwen2BF16InputConsumer(snapshot.path):
        pass
    assert seen == [{'meta'}]


def test_qwen2_bf16_renderer_roundtrips_all_fields_without_injected_frames(consumer):
...
def test_qwen2_bf16_full_runtime_fingerprint_rejects_each_untracked_mutation(consumer, drift):
    import torch
...
def test_qwen2_bf16_raw_bf16_comparison_distinguishes_signed_zero(consumer):
...
def test_qwen2_bf16_compile_and_execute_keep_all_fresh_scans_and_exact_accounting(consumer, monkeypatch):
    calls = []
...
def test_qwen2_bf16_real_tiny_bf16_generation_matches_independent_direct_dispatch(snapshot, consumer):
    torch = consumer._torch
...
def test_qwen2_bf16_private_copy_survives_source_deletion_and_closes_cleanly(snapshot, tmp_path):
    source = tmp_path / 'source'
    shutil.copytree(snapshot.path, source)
    with LocalQwen2BF16InputConsumer(source) as instance:
...
def test_qwen2_bf16_complete_prefix_counts_toward_exact_model_window(tmp_path):
    snapshot = create_qwen2_bf16_snapshot(tmp_path / "bounded", context_window=1024)
    with LocalQwen2BF16InputConsumer(snapshot.path) as consumer:
...
def _check_exact_window(consumer):
    ...
    rendered = consumer._tokenizer.apply_chat_template([{'role': 'user', 'content': 'x'}], tools=[],
        chat_template=QWEN2_BF16_CHAT_TEMPLATE, tokenize=False, add_generation_prompt=False)
    assert rendered.endswith('<|im_start|>assistant\n')


@pytest.mark.parametrize('kind', ['unused_row', 'malformed_shape', 'cache'])
def test_qwen2_bf16_generation_never_discards_invalid_output_or_cache(consumer, monkeypatch, kind):
    artifact = consumer.compile([{'role': 'user', 'content': 'prompt'}], output_reserved_tokens=4)
    def generate(**kwargs):
        if kind == 'cache':
...
@pytest.mark.parametrize('fault', ['extra_head', 'nonfinite'])
def test_qwen2_bf16_actual_loader_rejects_extra_head_or_nonfinite_even_with_consistent_manifest(snapshot, tmp_path, fault):
    import hashlib
    import json
    ...
    if fault == 'extra_head':
        shards[SHARD_FILES[0]]['lm_head.weight'] = state['model.embed_tokens.weight'].clone()
        index = json.loads((source / 'model.safetensors.index.json').read_text())
        index['weight_map']['lm_head.weight'] = SHARD_FILES[0]
        index['metadata']['total_size'] += state['model.embed_tokens.weight'].numel() * 2
        (source / 'model.safetensors.index.json').write_text(json.dumps(index))
    else:
        state['model.layers.0.self_attn.q_proj.bias'][-1] = float('nan')
    for name, values in shards.items():
        save_file(values, str(source / name))
    manifest = json.loads((source / 'snapshot.json').read_text())
    for name in (*SHARD_FILES, 'model.safetensors.index.json'):
        manifest['files'][name] = hashlib.sha256((source / name).read_bytes()).hexdigest()
    (source / 'snapshot.json').write_text(json.dumps(manifest))
    with pytest.raises(ModelInputError, match='snapshot admission' if fault == 'extra_head' else 'finite'):
        LocalQwen2BF16InputConsumer(source)
...
@pytest.mark.parametrize('drift', [
    ...
    'input_norm_eps', 'post_norm_eps', 'final_norm_eps',
    'attention_dropout', 'attention_causal', 'attention_window', 'attention_training', 'attention_backend',
])
def test_qwen2_bf16_cached_forward_metadata_drift_is_rejected_with_unchanged_tensors_and_serialized_config(
...
def test_qwen2_bf16_forward_metadata_is_rechecked_after_generation(consumer, monkeypatch):
    artifact = consumer.compile([{'role': 'user', 'content': 'prompt'}], output_reserved_tokens=4)
    def generate(**kwargs):
...
def test_qwen2_bf16_preserves_inactive_original_config_and_qkv_biases(consumer, snapshot):
    import json

    original = (snapshot.path / 'config.json').read_bytes()
    assert (consumer._snapshot.path / 'config.json').read_bytes() == original
    assert json.loads(original)['max_window_layers'] == 70
    config = consumer._model.config
    assert config.max_window_layers == 70 and config.sliding_window is None
    assert config.layer_types == ['full_attention'] * config.num_hidden_layers
    assert consumer._snapshot.config['sliding_window'] == 8192
    assert 'rope_scaling' not in consumer._snapshot.config and config.rope_scaling is None
    for layer in consumer._model.model.layers:
        assert all(projection.bias is not None for projection in (
            layer.self_attn.q_proj, layer.self_attn.k_proj, layer.self_attn.v_proj))
        assert layer.self_attn.o_proj.bias is None
        assert not hasattr(layer.self_attn, 'q_norm') and not hasattr(layer.self_attn, 'k_norm')


@pytest.mark.parametrize('mutation', ['loaded_bit', 'head_alias'])
def test_qwen2_bf16_loader_rejects_assignment_bit_change_or_false_alias(snapshot, monkeypatch, mutation):
    import torch
    from transformers import Qwen2ForCausalLM

    if mutation == 'loaded_bit':
        original = Qwen2ForCausalLM.load_state_dict

        def load_changed(self, state, *args, **kwargs):
            result = original(self, state, *args, **kwargs)
            # Replace storage before mutation so the retained source view is
            # independent and exposes a bad assignment, not a source edit.
            weight = self.model.layers[0].self_attn.q_proj.weight
            weight.data = weight.detach().clone()
            weight.data.view(torch.uint8).reshape(-1)[0] ^= 1
            return result

        monkeypatch.setattr(Qwen2ForCausalLM, 'load_state_dict', load_changed)
    else:
        original_tie = Qwen2ForCausalLM.tie_weights

        def false_alias(self, *args, **kwargs):
            original_tie(self, *args, **kwargs)
            self.lm_head.weight = torch.nn.Parameter(self.model.embed_tokens.weight.detach().clone(),
                                                     requires_grad=False)

        monkeypatch.setattr(Qwen2ForCausalLM, 'tie_weights', false_alias)
    with pytest.raises(ModelInputError, match='assigned learned values|tied weights'):
        LocalQwen2BF16InputConsumer(snapshot.path)


def test_qwen2_bf16_loader_finite_checks_are_chunk_bounded(snapshot, monkeypatch):
    import torch
    from daystrom_dml.services import qwen2_bf16_model_input as runtime

    monkeypatch.setattr(runtime, '_TENSOR_CHUNK_ELEMENTS', 128)
    ...
    with LocalQwen2BF16InputConsumer(snapshot.path) as loaded:
        assert observed
        assert sum(observed) == sum(value.numel() for value in loaded._model.parameters())


@pytest.mark.parametrize('projection', ['q_proj', 'k_proj', 'v_proj'])
def test_qwen2_bf16_missing_qkv_bias_refuses_before_generation(consumer, monkeypatch, projection):
    artifact = consumer.compile([{'role': 'user', 'content': 'prompt'}], output_reserved_tokens=4)
    getattr(consumer._model.model.layers[0].self_attn, projection).bias = None
    calls = []
    monkeypatch.setattr(consumer._model, 'generate', lambda **kw: calls.append(kw))
    with pytest.raises(ModelInputError, match='inventory|metadata'):
        consumer.execute(artifact)
    assert calls == []
