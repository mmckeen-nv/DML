...
def prepare_source(source: JournalStateStore) -> dict:
    if not source.path.is_file() or source.schema_version not in (2, 3, 4):
        raise ProjectionError("Projection source must be an existing schema-2/3/4 receipt journal")
