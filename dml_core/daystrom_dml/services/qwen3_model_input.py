"""Explicit CPU/BF16 Qwen3 companion with unchanged legacy consumers."""
...
from .qwen3_model_snapshot import (
    QWEN3_CHAT_TEMPLATE, SHARD_FILES, SPECIAL_TOKENS, expected_shapes, verify_qwen3_snapshot,
)
...
class LocalQwen3InputConsumer(LocalTransformersInputConsumer):
    ...
    def __init__(self, snapshot_directory: str | Path):
        ...
        self._closed = True
        try:
            self._snapshot_context = verify_qwen3_snapshot(snapshot_directory)
        except Exception as exc:
            raise ModelInputError("Local Qwen3 snapshot admission failed") from exc
        try:
            ...
            if self._template != QWEN3_CHAT_TEMPLATE:
                raise ModelInputError("Unsupported Qwen3 non-thinking template")
            ...
            if (len(self._allowed_ids) != len(vocabulary)
                ...
                raise ModelInputError("Qwen3 tokenizer vocabulary does not match model rows")
            for name in ("<|im_start|>", "<|im_end|>", "<|endoftext|>", "<think>", "</think>"):
                ...
                if (type(token) is not int or marker is None
                    ...
                    raise ModelInputError("Qwen3 framing marker is not one admitted token")
            if (config_data["eos_token_id"] != self._tokenizer.eos_token_id
                    or config_data["bos_token_id"] != self._tokenizer.pad_token_id):
                raise ModelInputError("Qwen3 configured special tokens differ")
            config = Qwen3Config.from_dict(config_data)
            config._attn_implementation = "eager"
            self._snapshot.validate_integrity()
            index = _json_object((self._snapshot.path / "model.safetensors.index.json").read_bytes(), "Qwen3 index")
            ...
            for shard in SHARD_FILES:
                ...
                if state.keys() & values.keys() or any(index["weight_map"].get(name) != shard for name in values):
                    raise ModelInputError("Qwen3 tensor shard ownership differs")
                state.update(values)
            shapes = expected_shapes(config_data)
            if state.keys() != shapes.keys() or state.keys() != index["weight_map"].keys():
                raise ModelInputError("Qwen3 learned tensor inventory differs")
            for name, shape in shapes.items():
                ...
                if (tuple(tensor.shape) != shape or tensor.device.type != "cpu" or tensor.dtype != torch.bfloat16
                    ...
                    raise ModelInputError("Qwen3 learned tensors require exact contiguous CPU BF16 shapes")
                ...
                if any(not bool(torch.isfinite(flat[start:start + _TENSOR_CHUNK_ELEMENTS]).all())
                    ...
                    raise ModelInputError("Qwen3 learned tensors must be finite")
            if not _raw_equal(state["lm_head.weight"], state["model.embed_tokens.weight"], torch):
                raise ModelInputError("Qwen3 supplied tied head bytes differ")
            ...
            self._model.model.rotary_emb = Qwen3RotaryEmbedding(config, device=torch.device("cpu"))
            ...
            if loaded.keys() != state.keys() or any(not _raw_equal(loaded[name], value, torch) for name, value in state.items()):
                raise ModelInputError("Qwen3 assigned learned values differ from verified sources")
            self._expected_shapes = shapes
            self._admitted_config = config
            self._norm_class = Qwen3RMSNorm
            self._rotary_class = Qwen3RotaryEmbedding
            ...
            if self._runtime_versions != dict(self._snapshot.manifest.runtime_versions):
                raise ModelInputError("Qwen3 runtime versions changed during initialization")
            ...
            if isinstance(exc, ModelInputError):
                raise
            raise ModelInputError("Verified Qwen3 model could not be loaded") from exc
    ...
    def _forward_metadata(self):
        ...
        if (model.config is not config or decoder.config is not config or config._attn_implementation != "eager"
            ...
            raise ModelInputError("Qwen3 forward metadata differs from admitted configuration")
        norms = [decoder.norm]
        layers = []
        for index, layer in enumerate(decoder.layers):
            ...
            if (attention.config is not config or mlp.config is not config
                ...
                raise ModelInputError("Qwen3 forward metadata differs from admitted configuration")
            norms.extend((attention.q_norm, attention.k_norm, layer.input_layernorm, layer.post_attention_layernorm))
    ...
    def _fingerprint(self):
        ...
        if (model.lm_head.weight is not model.model.embed_tokens.weight
            ...
            raise ModelInputError("Qwen3 tied weights or deterministic rotary aliases differ")
        parameters, buffers = dict(model.named_parameters()), dict(model.named_buffers())
        if parameters.keys() != self._expected_shapes.keys() - {"lm_head.weight"}:
            raise ModelInputError("Qwen3 runtime parameter inventory differs")
        if buffers.keys() != {"model.rotary_emb.inv_freq"}:
            raise ModelInputError("Qwen3 runtime buffer inventory differs")
        ...
        for group, values in (("parameter", parameters), ("buffer", buffers)):
            for name, value in values.items():
                expected_shape = self._expected_shapes[name] if group == "parameter" else (model.config.head_dim // 2,)
                ...
                if (value.device.type != "cpu" or value.dtype != expected_dtype or value.layout != torch.strided
                    ...
                    raise ModelInputError("Qwen3 runtime tensor left its admitted shape, dtype or layout")
    ...
    def compile(self, messages, tools=None, *, output_reserved_tokens):
        with self._lock:
            ...
            if any(token not in self._allowed_ids for token in artifact.input_ids):
                raise ModelInputError("Qwen3 input contains unused vocabulary rows")
            return artifact

    def execute(self, artifact):
        with self._lock:
            if getattr(self._model, "_cache", None) is not None:
                raise ModelInputError("Qwen3 cross-call cache state is excluded")
            result = super().execute(artifact)
            if getattr(self._model, "_cache", None) is not None:
                raise ModelInputExecutionError("Qwen3 generation retained forbidden cache state")
