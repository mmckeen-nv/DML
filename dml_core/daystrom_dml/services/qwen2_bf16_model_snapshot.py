"""Separate Qwen2 CPU/BF16 profile retaining original shards and absent-head alias.

The 3B source has 36 layers and an inactive max_window_layers of 70. These
fields are admitted here without changing any legacy CPU/F32 Qwen profile.
Context admission bounds tokens; it does not promise that full-context eager
attention will fit a particular host's memory.
"""
...
SNAPSHOT_SCHEMA_VERSION = "dml-qwen2-bf16-sharded-model-snapshot-v1"
CONSUMER_PROFILE = "qwen2-instruct-bf16-v1"
...
QWEN2_BF16_CHAT_TEMPLATE = QWEN_CHAT_TEMPLATE
QWEN2_BF16_CHAT_TEMPLATE_DIGEST = hashlib.sha256(QWEN2_BF16_CHAT_TEMPLATE.encode()).hexdigest()
...
_CONFIG_FIELDS = frozenset({"architectures", "attention_dropout", "bos_token_id",
    "eos_token_id", "hidden_act", "hidden_size", "initializer_range", "intermediate_size",
...
def expected_shapes(config: Mapping[str, Any]) -> dict[str, tuple[int, ...]]:
    """Stored learned tensors only; the absent head is a declared loader alias."""
...
def check_config(config: Mapping[str, Any], window: int) -> None:
    if set(config) != _CONFIG_FIELDS:
        raise base.SnapshotVerificationError("Unknown or incomplete Qwen2BF16 configuration")
    ...
    if any(type(config[key]) is not type(value) or config[key] != value for key, value in exact.items()):
        raise base.SnapshotVerificationError("Qwen2BF16 architecture is outside the admitted profile")
    for name in ("hidden_size", "intermediate_size", "num_hidden_layers", "num_attention_heads",
        ...
        if type(config[name]) is not int or not 1 <= config[name] <= 262144:
            raise base.SnapshotVerificationError("Invalid bounded Qwen2BF16 dimension")
    if (config["hidden_size"] % config["num_attention_heads"]
            or config["hidden_size"] // config["num_attention_heads"] % 2
            or config["num_attention_heads"] % config["num_key_value_heads"]
            or config["num_hidden_layers"] > 36 or type(window) is not int
            or not 1 <= window <= 32768 or not window <= config["max_position_embeddings"] <= 32768
            or config["sliding_window"] != config["max_position_embeddings"]
            or type(config["max_window_layers"]) is not int
            or not 0 <= config["max_window_layers"] <= 70):
        raise base.SnapshotVerificationError("Unsupported Qwen2BF16 attention or context dimensions")
    for name in ("bos_token_id", "eos_token_id"):
        if type(config[name]) is not int or not 0 <= config[name] < config["vocab_size"]:
            raise base.SnapshotVerificationError("Invalid Qwen2BF16 configured special token")
    for name in ("initializer_range", "rms_norm_eps", "rope_theta"):
        if type(config[name]) not in (int, float) or not math.isfinite(config[name]) or config[name] <= 0:
            raise base.SnapshotVerificationError("Invalid Qwen2BF16 scalar")
    if type(config["attention_dropout"]) not in (int, float) or config["attention_dropout"] != 0:
        raise base.SnapshotVerificationError("Qwen2BF16 inference dropout must be disabled")
    if type(config["transformers_version"]) is not str:
        raise base.SnapshotVerificationError("Invalid Qwen2BF16 producer version")
    if sum(math.prod(shape) for shape in expected_shapes(config).values()) > 3_100_000_000:
        raise base.SnapshotVerificationError("Qwen2BF16 model exceeds the admitted stored-parameter bound")
...
def read_regular(source: Path, destination: Path | None = None) -> tuple[bytes, str]:
    ...
    if limit is None:
        raise base.SnapshotVerificationError("Unlisted Qwen2BF16 artifact name")
    before = source.lstat()
    if (not stat.S_ISREG(before.st_mode) or before.st_nlink != 1 or before.st_size > limit):
        raise base.SnapshotVerificationError("Invalid Qwen2BF16 regular artifact or bounded size")
    descriptor = os.open(source, os.O_RDONLY | os.O_NOFOLLOW)
    output = None
    digest, captured = hashlib.sha256(), bytearray()
    with os.fdopen(descriptor, "rb") as handle:
        if _stable(os.fstat(handle.fileno())) != _stable(before):
            raise base.SnapshotVerificationError("Qwen2BF16 artifact changed while opening")
        try:
            ...
            while chunk := handle.read(_CHUNK_BYTES):
                ...
                if total > before.st_size:
                    raise base.SnapshotVerificationError("Qwen2BF16 artifact grew while reading")
                digest.update(chunk)
            ...
            if (total != before.st_size or _stable(source.lstat()) != _stable(before)
                ...
                raise base.SnapshotVerificationError("Qwen2BF16 artifact changed while reading")
            if output is not None:
                output.flush()
...
def validate_index(index: Mapping[str, Any], config: Mapping[str, Any]) -> dict[str, str]:
    ...
    if (type(index) is not dict or index.keys() != {"metadata", "weight_map"}
        ...
        raise base.SnapshotVerificationError("Qwen2BF16 index coverage, size or shard mapping differs")
...
def _check_header(path: Path, config: Mapping[str, Any], index: Mapping[str, str]) -> None:
    ...
    with os.fdopen(os.open(path, os.O_RDONLY | os.O_NOFOLLOW), "rb") as stream:
        if not stat.S_ISREG(before.st_mode) or _stable(os.fstat(stream.fileno())) != _stable(before):
            raise base.SnapshotVerificationError("Qwen2BF16 shard changed while opening header")
        prefix = stream.read(8)
        if len(prefix) != 8:
            raise base.SnapshotVerificationError("Truncated Qwen2BF16 safetensors header")
        length = struct.unpack("<Q", prefix)[0]
        if not 1 <= length <= _MAX_HEADER_BYTES:
            raise base.SnapshotVerificationError("Qwen2BF16 safetensors header exceeds bound")
        ...
        if metadata is not None and (type(metadata) is not dict
            ...
            raise base.SnapshotVerificationError("Invalid Qwen2BF16 safetensors metadata")
        names = {name for name, shard in index.items() if shard == path.name}
        if header.keys() != names:
            raise base.SnapshotVerificationError("Qwen2BF16 actual shard inventory differs from index")
        ...
        for name, value in header.items():
            if (type(value) is not dict or value.keys() != {"dtype", "shape", "data_offsets"}
                ...
                raise base.SnapshotVerificationError("Qwen2BF16 safetensors shape, dtype or offsets differ")
            start, end = value["data_offsets"]
            if start < 0 or end - start != 2 * math.prod(shapes[name]):
                raise base.SnapshotVerificationError("Invalid Qwen2BF16 safetensors payload length")
            intervals.append((start, end))
        cursor = 0
        for start, end in sorted(intervals):
            if start != cursor:
                raise base.SnapshotVerificationError("Qwen2BF16 safetensors payload gaps or overlaps")
            cursor = end
        if cursor + 8 + length != before.st_size:
            raise base.SnapshotVerificationError("Qwen2BF16 safetensors has trailing or incomplete payload")
        if _stable(path.lstat()) != _stable(before) or _stable(os.fstat(stream.fileno())) != _stable(before):
            raise base.SnapshotVerificationError("Qwen2BF16 shard changed during header verification")
...
def validate_state(state: Mapping[str, Any], config: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if sys.byteorder != "little":
        raise base.SnapshotVerificationError("Qwen2BF16 BF16 source bytes require little-endian CPU")
    shapes = expected_shapes(config)
    if state.keys() != shapes.keys():
        raise base.SnapshotVerificationError("Qwen2BF16 learned tensor coverage differs")
    records = []
    for name in sorted(shapes):
        ...
        if (not isinstance(tensor, torch.Tensor) or tensor.device.type != "cpu"
            ...
            raise base.SnapshotVerificationError("Qwen2BF16 tensor shape, dtype, device or layout differs")
        flat = tensor.detach().reshape(-1)
        for start in range(0, flat.numel(), _TENSOR_CHUNK_ELEMENTS):
            if not bool(torch.isfinite(flat[start:start + _TENSOR_CHUNK_ELEMENTS]).all()):
                raise base.SnapshotVerificationError("Qwen2BF16 source contains non-finite tensor values")
...
def load_qwen2_bf16_state(directory: Path, config: Mapping[str, Any], index: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for shard in SHARD_FILES:
        ...
        if values.keys() != {name for name, value in mapping.items() if value == shard} or state.keys() & values.keys():
            raise base.SnapshotVerificationError("Qwen2BF16 loaded shard inventory differs")
        state.update(values)
...
def _inventory(directory: Path, *, include_manifest: bool = True) -> None:
    ...
    if {entry.name for entry in directory.iterdir()} != expected:
        raise base.SnapshotVerificationError("Qwen2BF16 snapshot contains missing or unlisted artifacts")
    for name in expected:
        info = (directory / name).lstat()
        if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
            raise base.SnapshotVerificationError("Qwen2BF16 artifacts must be exclusive regular files")


def _directory(path: str | Path) -> tuple[Path, os.stat_result]:
    directory = Path(path).absolute()
    if ".." in directory.parts:
        raise base.SnapshotVerificationError("Qwen2BF16 directory traversal is excluded")
    for ancestor in (directory, *directory.parents):
        if not stat.S_ISDIR(ancestor.lstat().st_mode):
            raise base.SnapshotVerificationError("Qwen2BF16 directory symlinks are excluded")
...
def _manifest(payload: bytes) -> base.SnapshotManifest:
    ...
    if raw.keys() != base._MANIFEST_FIELDS or raw["schema_version"] != SNAPSHOT_SCHEMA_VERSION:
        raise base.SnapshotVerificationError("Unknown or incomplete Qwen2BF16 manifest schema")
    for name in ("model_id", "model_revision"):
        if type(raw[name]) is not str or not raw[name].strip() or len(raw[name].encode()) > 1024:
            raise base.SnapshotVerificationError("Invalid Qwen2BF16 source identity")
    if type(raw["context_window"]) is not int or not 1 <= raw["context_window"] <= 32768:
        raise base.SnapshotVerificationError("Invalid Qwen2BF16 effective context window")
    if (type(raw["files"]) is not dict or raw["files"].keys() != REQUIRED_FILES
        ...
        raise base.SnapshotVerificationError("Qwen2BF16 manifest files, versions or special tokens differ")
...
def _identity(manifest: base.SnapshotManifest, hashes: Mapping[str, str], versions: Mapping[str, str]):
    return ModelInputIdentity(
        ...
        runtime_identity="dml-qwen2-bf16-model-input-runtime-v1:" + base._canonical_digest({
            "versions": versions, "python": platform.python_version(), "device": "cpu", "dtype": "bfloat16",
            "model_class": "Qwen2ForCausalLM", "tokenizer_class": "PreTrainedTokenizerFast", "attention": "eager",
            "cache": "request-local-dynamic-v1", "decoder": "one-terminal-eos-only-v1",
            "weights": "original-shards-strict-assigned-declared-absent-head-alias-v1",
            "buffers": "deterministic-qwen2-rope-float32-v1", "fingerprint": "raw-bf16-full-scan-v1",
            "transport": "lossless-chatml-fields-v2"}), model_window_tokens=manifest.context_window)


class VerifiedQwen2BF16Snapshot(base.VerifiedSnapshot):
    @property
        ...
        try:
            ...
            for name, expected in self.manifest.files:
                ...
                if observed[name] != expected:
                    raise base.SnapshotVerificationError("Verified Qwen2BF16 artifact changed")
                if name == "config.json" and payload != self._config_bytes:
                    raise base.SnapshotVerificationError("Verified Qwen2BF16 configuration cache differs")
                if name == "chat_template.jinja" and payload != self._chat_template.encode():
                    raise base.SnapshotVerificationError("Verified Qwen2BF16 template cache differs")
            versions = base._check_runtime(self.manifest)
            if self.identity != _identity(self.manifest, observed, versions):
                raise base.SnapshotVerificationError("Verified Qwen2BF16 identity changed")
        except OSError as exc:
            raise base.SnapshotVerificationError("Cannot verify private Qwen2BF16 artifacts") from exc


def verify_qwen2_bf16_snapshot(directory: str | Path) -> VerifiedQwen2BF16Snapshot:
    temporary = None
    try:
        ...
        temporary = tempfile.TemporaryDirectory(prefix="dml-qwen2-bf16-input-")
        target = Path(temporary.name)
        contents, observed = {}, {}
        for name, expected in manifest.files:
            contents[name], observed[name] = read_regular(source / name, target / name)
            if observed[name] != expected:
                raise base.SnapshotVerificationError("Qwen2BF16 artifact hash mismatch")
        ...
        if final != manifest_bytes or (current.st_dev, current.st_ino) != (initial.st_dev, initial.st_ino):
            raise base.SnapshotVerificationError("Qwen2BF16 source directory changed while copying")
        ...
        if contents["chat_template.jinja"] != QWEN2_BF16_CHAT_TEMPLATE.encode():
            raise base.SnapshotVerificationError("Unsupported Qwen2BF16 non-thinking template")
        for name in REQUIRED_FILES:
            (target / name).chmod(0o400)
        target.chmod(0o500)
        return VerifiedQwen2BF16Snapshot(temporary, manifest, _identity(manifest, observed, versions),
            contents["config.json"], QWEN2_BF16_CHAT_TEMPLATE, _verification_token=base._VERIFICATION_TOKEN)
