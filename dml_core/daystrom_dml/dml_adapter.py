...
class DMLAdapter:
    ...
    def _require_legacy_mutations(self) -> None:
        if self._journal is not None and self._journal.schema_version in (2, 3, 4):
            raise ValueError("Receipt-journal adapters support only receipted append and retirement mutations; legacy writes and unqualified lifecycle changes are unavailable")
